...
def insert_records(conn, table_name: str, records_list: List[Dict[str, Any]], delete_recs: bool=False):
    """
...
def upsert_records(conn, table_name: str, conflict_col: str, records_list: List[Dict[str, Any]]):
    """
        On conflict, update records based on the conflict column (key) in the table

        Params
        ------
            * conn: psycopg2 connection object
            * table_name: name of the table to upsert
            * conflict_col: the key of the table
            * records_list: list of dicts keyed by column name of value to insert
                for that column
    """
    cur = conn.cursor()
    ...
    for record in records_list:
        columns = [f'"{x}"' for x in record.keys()]
        rep_cons = AsIs(",".join([f"{x}='{record[x]}'" for x in record.keys() if x != conflict_col]))
        values = record.values()
        sql = """
            insert into %s (%s) values %s on conflict(%s) do update set %s
        """
        formatted_sql = cur.mogrify(
            sql,
            (
                AsIs(table_name),
                AsIs(",".join(columns)),
